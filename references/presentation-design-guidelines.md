...
> 基于 v6.2 实战验证。竖版暗色影视风 · 中英双语 · 手机比例。
...
| 元素 | 字号 | 说明 |
|------|------|------|
| 封面标题 | 2.4rem | 后跟v6.2版本号小标 |
| 页面标题 h2 | 1.35rem | |
| 大师名称 | 0.88rem | 加粗+暖金 |
| 大师技能描述 | 0.7rem | 正文主色 |
| 统计数字 | 1.5rem | 卡片内大数字 |
| 表格正文 | 0.64rem | |
| flow/竖排内容 | 0.64rem | line-height:1.35 |
| 高亮引用 | 0.78rem | |
| 页脚 | 0.45rem | rgba(255,255,255,0.13) — 不喧宾夺主 |
| 页脚过小 | ❌ 0.24rem | 太小看不清，0.45rem是甜点位 |

## 硬性死规则（v6.2实战教训）

| 规则 | 如果不遵守 |
|------|-----------|
| slide必须 `border:none` 不能有边框 | PDF会在页面边缘留白线 |
| 必须去掉 `::before` 金线伪元素 | Chrome渲染为黄线，用户反复投诉 |
| 每页 `max-height:844px; overflow:hidden` | 内容超出会产生空白残页 |
| flow必须竖排（`<br>`换行），禁用ASCII树 | 横向`├─│└─`撑出页面宽出滑道 |
| 每次改完HTML必须重跑Chrome headless生成PDF | 不跑就是旧版，改了白改 |
| 版本号全域同步 `grep -rn v5\\.[0-9]` 零差异 | README/SKILL/references各说各话 |
| card内`.info`必须 `flex:1; min-width:0` | 长英文撑出卡片溢出 |
| 页脚`position:absolute; bottom:6px` | 不设absolute就跟着内容走，不在页底 |
