...
> v6.1 十轮迭代积累的全部教训。改 PPT 前必读，每一项都是踩过的坑。
...
**必保规则清单**：`.footer-bar` `.slide` `.master-detail` `.flow` `.highlight` `.table` `.stat`
...
**正确**：每个步骤独立一行
```
P1 剧本<br><span>6步工作流</span><br>
P2 升级<br><span>武打+暗线+65s</span><br>
```
...
| 页脚 .footer-bar | **0.45rem** | `rgba(255,255,255,0.13)` | 可见但不抢眼，不可再小 |
